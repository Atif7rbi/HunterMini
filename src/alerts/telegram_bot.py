...
class TelegramAlerter:
    ...
    async def setup_alert(self, card: TradeCard) -> None:
        ...
        msg_lines = [
            ...
            f"🕐 <i>{ts}</i>",
            SEP,
            "<i>HunterMini · Validation Sandbox</i>",
        ]
    ...
    # ------------------------------------------------------------------
    async def exit_alert(self, trade: Trade, reason: str = "") -> None:
        """Send a compact close alert.

        Uses the persisted Trade object so the Telegram message matches
        the Closed Trades table: symbol, direction, entry, exit, P/L, R,
        status, reason, and timestamps.

        Display-only notification:
        - no strategy changes
        - no score changes
        - no trade-management changes
        """
        ...
        status_clean = status.rsplit(".", 1)[-1].replace("CLOSED_", "")

        entry = (
            trade.actual_entry_price
            or (
                ((trade.entry_zone_low or 0.0) + (trade.entry_zone_high or 0.0)) / 2
                if trade.entry_zone_low and trade.entry_zone_high
                else 0.0
            )
        )
        exit_price = float(trade.exit_price or 0.0)
        pnl_usd = float(trade.pnl_usd or 0.0)
        pnl_r = float(trade.pnl_r or 0.0)

        result_emoji = "🟢" if pnl_usd >= 0 else "🔴"
        sign = "+" if pnl_usd >= 0 else ""

        arrow = _dir_arrow(direction)
        reason_text = html.escape(reason or status_clean or "Closed")

        opened_at = trade.triggered_at or trade.created_at
        closed_at = trade.closed_at or _now()

        opened_txt = (
            opened_at.strftime("%Y-%m-%d %H:%M UTC")
            if opened_at
            else "—"
        )
        closed_txt = (
            closed_at.strftime("%Y-%m-%d %H:%M UTC")
            if closed_at
            else _now().strftime("%Y-%m-%d %H:%M UTC")
        )

        msg_lines = [
            f"🏁 <b>{html.escape(symbol)}</b>  {arrow}  <b>{html.escape(direction)}</b>",
            SEP,
            f"{result_emoji} <b>CLOSED — {html.escape(status_clean)}</b>",
            "",
            SEP,
            "<b>💰 RESULT</b>",
            SEP,
            f"🔵 <b>Entry:</b>  <code>{float(entry or 0.0):.6g}</code>",
            f"⚪ <b>Exit:</b>   <code>{exit_price:.6g}</code>",
            "",
            f"💵 <b>P/L:</b>    <code>{sign}${pnl_usd:.2f}</code>",
            f"⚖️ <b>R:</b>      <code>{pnl_r:+.2f}R</code>",
            "",
            SEP,
            "<b>📌 CLOSE REASON</b>",
            SEP,
            f"<code>{reason_text}</code>",
            "",
            SEP,
            "<b>⏱ TIME</b>",
            SEP,
            f"Opened: <code>{opened_txt}</code>",
            f"Closed: <code>{closed_txt}</code>",
            "",
            "<i>HunterMini · Validation Sandbox</i>",
        ]
    ...
    async def weekly_report(self) -> None:
        ...
        msg_lines += ["", "<i>HunterMini · Validation Sandbox</i>"]
        await self._send("\n".join(msg_lines), "REPORT", "system")
